 ...
 *      takeOut(int money);
 * };
 * 6. 类构造函数在类进行初始化的时候被调用；类析构函数在对象过期的时候被调用
 * 7. 见编程练习1的具体实现
 * 8. 默认构造函数是编译器自动生成的构造函数，在没有进行任何构造函数的定义时将被自动调用；
 *    好处：可以声明对象而不用初始化它
 * 9. ？数据成员有多个，怎么返回各个数据成员值   三个成员变量，那就写三个成员函数分别将他们返回就行了，注意const
 * 10. this是指向调用对象的地址， *this是该调用对象的别名； this是对象的地址，*this是对象本身
*/
...
// 8. mian(), cha10_List.h, cha10_List.cpp
